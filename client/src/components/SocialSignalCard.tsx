...
export type SignalEntry = {
  signal: { id: number; body: string; type: string; visibility: string; publishedAt: Date | string | null; imageUrl?: string | null; imageAspect?: "wide" | "square" | null };
  profile: Profile;
...
export default function SocialSignalCard({ entry, activeProfileId, onRefresh, compact = false }: { entry: SignalEntry; activeProfileId?: number; onRefresh?: () => void; compact?: boolean }) {
  ...
  const { signal, profile, comments } = entry;
  ...
  return <article className={`rounded-2xl border border-white/[.08] bg-[linear-gradient(145deg,rgba(18,28,48,.6),rgba(8,12,23,.66))] p-4 shadow-[0_14px_32px_rgba(0,0,0,.12)] ${compact ? "" : "sm:p-5"}`}>
    <div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-cyan-200/20 bg-gradient-to-br from-cyan-100 via-sky-300 to-violet-400 text-xs font-extrabold text-[#07101f]">{initials(profile.displayName)}</div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-bold text-white">{profile.displayName}</p><p className="mt-0.5 text-[10px] font-bold uppercase tracking-[.12em] text-slate-500">@{profile.username} · {formatDate(signal.publishedAt)}</p></div><span className="rounded-full border border-white/[.08] px-2 py-1 text-[9px] font-extrabold uppercase tracking-[.1em] text-slate-600">{signal.type}</span></div>{signal.imageUrl && <div className={`mt-4 overflow-hidden rounded-xl border border-white/[.09] bg-slate-950/55 ${signal.imageAspect === "square" ? "aspect-square" : "aspect-video"}`}><img src={signal.imageUrl} alt={signal.body || `${profile.displayName}'s Signal image`} className="h-full w-full object-cover" loading="lazy" /></div>}{signal.body && <p className="mt-4 whitespace-pre-wrap text-sm leading-6 text-slate-300">{signal.body}</p>}</div></div>
