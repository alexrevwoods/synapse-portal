...
type EditableSignal = {
  ...
  mediaLicense?: "all_rights_reserved" | "credit_required" | "collaboration_allowed";
...
export default function SignalEditor({ profileId, username, signal, onSaved }: { profileId: number; username: string; signal: EditableSignal; onSaved?: () => void }) {
  ...
  const [mediaLicense, setMediaLicense] = useState(signal.mediaLicense || "all_rights_reserved");
  ...
  useEffect(() => {
    ...
    setMediaLicense(signal.mediaLicense || "all_rights_reserved");
  }, [open, signal.body, signal.mediaLicense, signal.seoDescription, signal.seoImageUrl, signal.seoTitle]);

  const save = () => update.mutate({ profileId, signalId: signal.id, body, seoTitle, seoDescription, seoImageUrl, mediaLicense });
  ...
  return <div className="mt-4 border-t border-white/[.08] pt-3">
    ...
    {open && <div className="mt-3 space-y-3 rounded-xl border border-cyan-200/16 bg-cyan-300/[.035] p-3.5">
      <label className="block text-[10px] font-extrabold uppercase tracking-[.1em] text-slate-500">Signal content<textarea value={body} onChange={(event) => setBody(event.target.value)} maxLength={5000} rows={4} className="mt-1.5 w-full resize-y rounded-lg border border-white/[.1] bg-slate-950/55 px-3 py-2.5 text-sm font-normal normal-case tracking-normal text-white outline-none placeholder:text-slate-600 focus:border-cyan-200/50" /></label><fieldset className="rounded-lg border border-white/[.08] bg-slate-950/30 p-3"><legend className="px-1 text-[10px] font-extrabold uppercase tracking-[.1em] text-violet-200">Media & rights</legend><label className="block text-[10px] font-bold uppercase tracking-[.08em] text-slate-500">License<select value={mediaLicense} onChange={(event) => setMediaLicense(event.target.value as typeof mediaLicense)} className="mt-1.5 h-9 w-full rounded-lg border border-white/[.1] bg-[#08101A] px-2.5 text-xs font-normal normal-case tracking-normal text-white outline-none focus:border-violet-200/50"><option value="all_rights_reserved">All rights reserved — reuse requires permission</option><option value="credit_required">Credit required — request permission first</option><option value="collaboration_allowed">Collaboration welcome — contact the owner</option></select></label></fieldset>
