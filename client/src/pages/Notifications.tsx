...
export default function Notifications() {
  ...
  const acceptConnection = trpc.relationships.acceptConnection.useMutation({ onSuccess: () => { toast.success("Connection accepted"); network.refetch(); notifications.refetch(); }, onError: (error) => toast.error(error.message) });
  ...
  if (profile.isLoading || notifications.isLoading || network.isLoading) return <main className="min-h-screen bg-[#080B14] p-10 text-sm font-bold text-slate-500">Loading notifications…</main>;
