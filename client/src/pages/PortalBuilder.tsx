    ...
    <main className="min-h-screen bg-[#070b14] text-white">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[17rem_minmax(0,1fr)] lg:px-8">
        ...
        <div className="space-y-6">
          ...
          <PortalThemePicker profileId={profileId} currentTheme={profile.portalTheme} brandPrimary={profile.brandPrimaryColor} brandSecondary={profile.brandSecondaryColor} onChange={refresh} />
          <section className="rounded-2xl border border-white/[.09] bg-slate-950/30 p-5 sm:p-7"><div className="flex flex-col gap-3 border-b border-white/[.08] pb-5 sm:flex-row sm:items-center sm:justify-between"><div><span className="eyebrow">Connected Nodes</span><h2 className="font-display mt-3 text-2xl font-semibold tracking-[-.05em] text-white">Map what surrounds this identity.</h2><p className="mt-2 text-sm leading-6 text-slate-500">Each Node is a reusable identity object—never just a button in a list.</p></div><div className="flex gap-2"><Link href={`/brand/${profileId}`} className="secondary-button !px-4 !py-2.5 !text-xs"><Globe2 size={14} /> Brand Studio</Link><button onClick={() => setShowNodeForm((current) => !current)} className="primary-button !px-4 !py-2.5 !text-xs"><Plus size={14} /> Add Node</button></div></div>
